...
module.exports = context => {
  ...
  if (
    ...
    (context.hook === "after_plugin_add" &&
      context.cmdLine.includes("plugin add"))
  ) {
    getPlatformVersionsFromFileSystem(context, projectRoot).then(
      platformVersions => {
        const IOS_MIN_DEPLOYMENT_TARGET = "7.0";
        const platformPath = path.join(projectRoot, "platforms", "ios");
        const config = getConfigParser(
          context,
        ...
        bridgingHeaderPath = getBridgingHeaderPath(
          projectPath,
          iosPlatformVersion
        );
        ...
        const bridgingHeaderProperty =
          '"$(PROJECT_DIR)/$(PROJECT_NAME)' +
          bridgingHeaderPath.split(projectPath)[1] +
          '"';
          ...
          for (configName in buildConfigs) {
            if (!COMMENT_KEY.test(configName)) {
              ...
              if (
                ...
                console.log(
                  "Update IOS build setting LD_RUNPATH_SEARCH_PATHS to: @executable_path/Frameworks",
                  "for build configuration",
                  buildConfig.name
                );
              }
              ...
              if (
                typeof xcodeProject.getBuildProperty(
                  "SWIFT_VERSION",
                  buildConfig.name
                ) === "undefined"
              ) {
                if (
                  ...
                  const swiftVersion = config.getPreference(
              ...
              if (buildConfig.name === "Debug") {
                if (
                  ...
                  console.log(
                    "Update IOS build setting SWIFT_OPTIMIZATION_LEVEL to: -Onone",
                    "for build configuration",
                    buildConfig.name
                  );
                }
...
const getBridgingHeaderPath = (projectPath, iosPlatformVersion) => {
  ...
  if (semver.lt(iosPlatformVersion, "4.0.0")) {
    bridgingHeaderPath = path.posix.join(
      projectPath,
      "Plugins",
      "Bridging-Header.h"
    );
